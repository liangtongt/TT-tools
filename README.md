# 🎭 TT img - 图片视频加密隐写ComfyUI节点

> 🚀 **强大的文件加密隐写技术，让文件传输更安全！**
...
🎯 **核心特性**：Comfyui中将视频、图片等文件隐藏到普通图片的像素信息中，实现文件加密，可自由选择无密码节点(解码更快)或有密码节点。本地可通过exe解码，无需comfyui环境，CPU即可快速解码

🛡️ **安全优势**：
- 🔒 **高级加密**：图片看起来完全正常，提供强大的数据保护
- 🔑 **自定义密码**：支持设置自定义密码，增强文件安全性
...
TT img 是一个完整的图片视频加密隐写comfyui插件，包含编码和解码两个核心组件：
...
2. **加密图片生成**：创建带有随机数据的加密图片，完美伪装

3. **文件嵌入**：将转换后的文件（MP4/JPG）加密嵌入到图片中

### 🔓 TT img dec（解码节点）
1. **文件提取**：从加密图片中提取隐藏的文件（MP4/JPG等）
2. **自动识别**：自动识别文件类型和扩展名
3. **智能解码**：智能提取数据
4. **静默运行**：完美还原原始文件
...
- **password**: 自定义密码（可选，默认为空，增强文件安全性）
...
- **password**: 解密密码（如果编码时设置了密码，解码时必须输入相同密码）
...
3. 设置输出文件名（可选）
4. 运行工作流
5. 提取的文件自动保存在ComfyUI的 `output/` 目录下
6. 自动重命名避免文件覆盖
...
**🐍 方法3：使用Python脚本**
```bash
python tt_img_dec_loc.py <图片路径> [密码]
```

**📂 方法4：指定输出路径**
```bash
python tt_img_dec_loc.py <图片路径> <输出文件路径> [密码]
```
...
python tt_img_dec_loc.py output_image.png my_video.mp4 mypassword123
```
...
TT img 是一个功能强大的图片加密隐写工具套件，专为安全文件传输而设计：

### 🔒 **核心优势**
- 🔐 **高级加密**：图片看起来完全正常，提供强大的数据保护
- 🔑 **密码保护**：支持自定义密码，确保只有授权用户能访问文件
